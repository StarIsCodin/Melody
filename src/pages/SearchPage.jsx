...
const SearchPage = () => {
    ...
    const timer = setTimeout(() => {
      if (searchTerm.trim()) {
        ...
        const fetchAllSongs = async () => {
          try {
            const response = await axios.get('https://melody-api-lh84.onrender.com/api/songs');
  ...
  const searchSongs = async (query) => {
    setIsSearching(true);
    try {
      const response = await axios.get(`https://melody-api-lh84.onrender.com/api/songs/search?query=${query}`);
      setMusicItems(response.data);
    } catch (err) {
